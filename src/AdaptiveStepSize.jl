...
A singular point is such that the function is not defined, like `log(x)` at x = 0. We can also
use it for points where the second derivative is not continuous. For example `abs(x)` at x = 0.

If the function is an piecewise function, we can write in the `singularities` vector those
discontinuous points. The function `points_linear` will be applied at each subinterval. And
...
handle those discontinuities properly. It is the user responsability to do so, otherwise,
call it with a value slightly larger or smaller.
"""
